...
pub const Function = struct {
    ...
    pub const arch = std.Target.Cpu.Arch.spu_2;
    ...
    fn genCall(self: *Self, inst: *ir.Inst.Call) !MCValue {
        ...
        if (self.bin_file.tag == link.File.Elf.base_tag or self.bin_file.tag == link.File.Coff.base_tag) {
            if (inst.func.value()) |func_value| {
                if (info.args.len != 0) {
                    return CodegenUtils.fail(Self, self, inst.base.src, "TODO implement call with more than 0 parameters", .{});
                }
                if (func_value.castTag(.function)) |func_payload| {
                    ...
                    if (return_type.zigTypeTag() == .NoReturn) {
                        ...
                        switch (return_type.zigTypeTag()) {
                            ...
                            else => return CodegenUtils.fail(Self, self, inst.base.src, "TODO implement fn call with non-void return value", .{}),
                        }
                    }
    ...
    fn ret(self: *Self, src: LazySrcLoc, mcv: MCValue) !MCValue {
        ...
        return CodegenUtils.fail(Self, self, src, "TODO implement return for {}", .{self.target.cpu.arch});
    }
    ...
    fn genAsm(self: *Self, inst: *ir.Inst.Assembly) !MCValue {
        ...
        if (inst.inputs.len > 0 or inst.output != null) {
            return CodegenUtils.fail(Self, self, inst.base.src, "TODO implement inline asm inputs / outputs for SPU Mark II", .{});
        }
        if (mem.eql(u8, inst.asm_source, "undefined0")) {
            ...
            return CodegenUtils.fail(Self, self, inst.base.src, "TODO implement support for more SPU II assembly instructions", .{});
        }
    }
    ...
    pub fn genSetReg(self: *Self, src: LazySrcLoc, ty: Type, reg: Register, mcv: MCValue) InnerError!void {
        return CodegenUtils.fail(Self, self, src, "TODO implement getSetReg for {}", .{self.target.cpu.arch});
    }
